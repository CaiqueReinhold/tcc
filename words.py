...
FILES_PATH = path.join('/', 'home', 'caique', 'Downloads', 'words')
...
def load_img(img):
    ...
    if img.shape[1] % 10 != 0:
        pad = np.zeros((100, img.shape[1] % 10), dtype=theano.config.floatX)
...
def main():
    with open(path.join(FILES_PATH, 'lines.txt')) as file:
        ...
        print np.mean([pre_process_img(item) for item in data])
